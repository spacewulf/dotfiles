...
GpuAdapters.AVAILABLE_BAKCENDS = {
...
function GpuAdapters:init()
	local initial = {
		__backends = self.AVAILABLE_BAKCENDS[platform.os],
		__preferred_backend = self.AVAILABLE_BAKCENDS[platform.os][1],
		DiscreteGpu = nil,
		IntegratedGpu = nil,
		Cpu = nil,
		Other = nil,
	...
	---Iterate over the enumerated GPUs adn create a lookup table (`AdapterMap`)
...
--- 2. Best graphics API available (based of my very scientific scroll a bit log file in neovim test)
---
--- Graphics API choices are based on the platform:
---  - Windows: Dx12 > Vulkan > OpenGL
---  - Linux: Vulkan > OpenGl
---  - Mac: Metal
---@see GpuAdapters.AVAILABLE_BAKCENDS
---
--- If the best adapter combo is not found, it will return `nil` and lets Wezterm decide the best adapter.
---
--- Please note these are my own personal preferences and may not be the best for your sytem.
--- If you want to manually choose the adapter, use `GpuAdapters:pick_manual(backend, deice_type)`
--- Or feel free to re-arrange `GpuAdapters.AVAILABLE_BAKCENDS` to your liking
---
